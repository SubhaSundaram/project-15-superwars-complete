...
class Superwar {
  ...
  buildPlayers = (type) => {
    ...
    this.filterPlayers(type).forEach((player) => fragment.append(player.view()))
    return fragment
  }
  ...
  fight = () => {
    ...
    let filtplayer = this.players.filter((player) => player.selected)
    if (filtplayer[0].strength > filtplayer[1].strength) {
      filtplayer[0].strength = filtplayer[0].strength - filtplayer[1].strength
      filtplayer[1].strength = 0
      filtplayer[0].wins += 1
    } else {
      filtplayer[1].strength = filtplayer[1].strength - filtplayer[0].strength
      filtplayer[0].strength = 0
      filtplayer[1].wins += 1
    }
  ...
  checkWin = () => {
    ...
    // Type your code here
    let result
    if (this.totalStrength('villain') == 0) result = 'hero'
    else if (this.totalStrength('hero') == 0) result = 'villain'
    else result = 'endure'
    return result
  }
  ...
  totalStrength = (type) => {
    ...
    // Type your code here
    let strength = this.filterPlayers(type)
      .map((x) => x.strength)
      .reduce(function (acc, cv) {
        return acc + cv
      })
  ...
  announceWinner = (score) => {
    console.log('Announcement block')
